...
/usr/bin/curl -i -H "Content-Type: application/json" -X POST -d '{"user": "bob": "email": "bob@example.com"}' http://server-demo:8000/data > response.txt
...
/usr/bin/curl -i -H "Content-Type: application/json" -X POST -d '{"user": "bob": "email": "bob@example.com"}' http://server-demo:8000/unknown  > response.txt
